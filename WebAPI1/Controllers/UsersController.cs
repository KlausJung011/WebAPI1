...
namespace WebAPI1.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        [HttpPut("updateemail")]
        public async Task<IActionResult> UpdateEmail(UpdateUserEmailDto dto)
        {
            try
            {
                var user = await _userService.UpdateEmailAsync(dto.Id, dto.Email!);
                return Ok(new { message = "Email actualizado", user });
            }
            catch (Exception ex)
            {
                return BadRequest(new { error = ex.Message });
